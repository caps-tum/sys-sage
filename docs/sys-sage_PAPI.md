...
library. Note that `hwloc` is required as a dependency. Please make sure that
it's installed on your system and provide an XML file generated by `hwloc`,
which captures the hardware topology, to `sys-sage::parseHwlocOutput`.
...
provided that envelop the underlying PAPI routine. The wrapper functions are
...
routines like `PAPI_create_eventset`, `PAPI_attach` and so on. However, the
wrappers `SS_PAPI_start`, `SS_PAPI_reset` and `SS_PAPI_accum` must be used
instead of `PAPI_start`, `PAPI_reset` and `PAPI_accum` respectively.
...
More sophisticated examples can be found in the `examples/` directory of this
repository. The examples include
