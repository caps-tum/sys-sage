...
namespace sys_sage {
   ...
   *                object of category `PAPI_Metrics`. The pointer must not be
   *                `nullptr`. If the pointer that is being pointed to is
   *                `nullptr`, a new Relation object is created and `*metrics`
   *                is set accordingly. Otherwise, `*metrics` is reused.
   *
   * @return If `metrics == nullptr` then `PAPI_EINVAL` is returned, otherwise
   *         the same as in `PAPI_start`.
   *         .
   */
   ...
   *                eventSet.
   *
   * @return The same as in `PAPI_reset`.
   */
   ...
   *
   * @param metrics The relation of category `PAPI_Metrics` that stores the
   *                perf counter values.
   ...
   */
  long long GetCpuPerfVal(Relation *metrics, int event, int cpuNum = -1, unsigned long long timestamp = 0);
   ...
   */
  CpuPerf *GetCpuPerf(Relation *metrics, int event, int cpuNum);
